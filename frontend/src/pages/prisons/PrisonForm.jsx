...
export const PrisonForm = () => {
  ...
  useEffect(() => {
    const officers = getOfficers().filter(o => o.role === 'prison_manager');
    setManagers(officers);

    if (id) {
      const result = getPrisonDetail(id); // also import getPrisonDetail
      if (result) {
        setFormData({
          ...result.prison,
          infirmary: result.features.infirmary,
          workshops: result.features.workshops,
          agricultural_ward: result.features.agricultural_ward,
          visitation_hall: result.features.visitation_hall,
          visitation_hall_capacity: result.features.visitation_hall_capacity,
