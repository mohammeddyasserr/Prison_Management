...
async def api_superadmin_dashboard(request: Request):
    ...
    return JSONResponse({
        ...
        "transfer_stats": dict(transfer_stats) if transfer_stats else {"pending": 0, "approved": 0, "denied": 0},
        "alerts": rows_to_dicts(alerts),
        "high_risk": rows_to_dicts(high_risk)
    })

@router.get("/api/dashboard/manager")
async def api_manager_dashboard(request: Request):
    user = get_current_user(request)
    if not user or user["role"] != "prison_manager":
        return JSONResponse({"error": "Unauthorized"}, status_code=401)
    ...
    return JSONResponse({
        ...
        "active_incidents": dict(active_incidents) if active_incidents else {"count": 0},
        "pending_visits": rows_to_dicts(pending_visits),
        "upcoming_releases": rows_to_dicts(upcoming_releases),
        "pending_transfers": rows_to_dicts(pending_transfers)
    })


@router.get("/api/dashboard/officer")
async def api_officer_dashboard(request: Request):
    user = get_current_user(request)
    if not user or user["role"] != "officer":
        return JSONResponse({"error": "Unauthorized"}, status_code=401)

    db = get_db()
    officer_id = user["national_id"]
    prison_id = user["prison_id"]
    ...
    """, (officer_id, datetime.now().strftime("%Y-%m-%d"))).fetchall()

    assigned_block_ids = [b["block_id"] for b in assigned_blocks]

    # Cells in those blocks
    cells = []
    ...
    return JSONResponse({
        "assigned_blocks": rows_to_dicts(assigned_blocks),
        "cells": rows_to_dicts(cells),
        "recent_incidents": rows_to_dicts(recent_incidents),
        "active_solitary": rows_to_dicts(active_solitary),
        "my_shifts": rows_to_dicts(my_shifts)
    })
